...
class ArmParameters:
    ...
    def __init__(self, nbJoint, nbMuscle):
        ...
        '''
        self.pathSetupFile = os.getcwd() + "/ArmModel/Setup/ArmParams"+str(nbJoint)+str(nbMuscle)+".xml"
    ...
    def readStops(self,tree):
        self.lowerBounds=np.empty(self.nbJoint)
        self.upperBounds=np.empty(self.nbJoint)
        for i, parts in enumerate(tree):
            self.lowerBounds[i]=float(parts[0].text)
            self.upperBounds[i]=float(parts[1].text)
